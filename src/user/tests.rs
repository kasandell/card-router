...
mod tests {
    ...
    #[test]
    async fn test_get_or_create_throws_on_create() {
        crate::test_helper::general::init();
        let footprint_response = CreateUserVaultResponse {
            id: FOOTPRINT_ID.to_string()
        };
        ...
        user_dao.expect_find_by_auth0_id()
            .times(1)
            .return_once(move |_| Err(DataError::NotFound("Not found".into())));

        user_dao.expect_create()
            .times(1)
            .return_once(move |_| Err(DataError::Unexpected("could not insert".into())));

        let mut service = Arc::new(UserService::new_with_mocks(
            Arc::new(user_dao),
            Arc::new(fp_mock)
        ));
    ...
    async fn test_find_by_internal_id_errors_unexpected() {
        ...
        let service = Arc::new(UserService::new_with_mocks(
            Arc::new(user_mock),
            Arc::new(fp_mock)
        ));
        let error = service.clone().find_by_internal_id(1000028).await.expect_err("Should not find");
