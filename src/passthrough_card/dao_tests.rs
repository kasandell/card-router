...
mod tests {
    ...
    async fn test_create_insertable_card() {
        ...
        let dao = Arc::new(PassthroughCardDao::new());
        let created_card = dao.clone().create(
            insertable_card
    ...
    async fn test_create_insertable_card_fails_for_same_token() {
        ...
        let insertable_card = InsertablePassthroughCard::try_from((card, &user)).expect("converts");
        let created_card = dao.clone().create(insertable_card.clone()).await.expect("inserts");
        assert_eq!(user.id, created_card.user_id);
        assert_eq!(expected_date, created_card.expiration);
        assert_eq!(PassthroughCardStatus::Open, created_card.passthrough_card_status);
        assert_eq!(PassthroughCardType::Virtual, created_card.passthrough_card_type);
        ...
        let dao = Arc::new(PassthroughCardDao::new());
        let created_error = dao.clone().create(
    ...
    async fn test_create_insertable_card_fails_for_active_already() {
        ...
        let insertable_card = InsertablePassthroughCard::try_from((card, &user)).expect("converts");
        let created_card = dao.clone().create(insertable_card.clone()).await.expect("creates");
        ...
        let created_error = dao.clone().create(insertable_card.clone()).await.expect_err("conflicts");
    ...
    async fn test_list_card_for_user() {
        ...
        let insertable_card2 = InsertablePassthroughCard::try_from((card2, &user)).expect("converts");
        let created_card2 = dao.clone().create(insertable_card2).await.expect("creates");
        ...
        let created_card3 = dao.clone().create(insertable_card3).await.expect("creates");

        let cards = dao.clone().find_cards_for_user(user.id).await.expect("no error");
        assert_eq!(
    ...
    #[test]
    async fn test_find_for_user_in_status_finds() {
        crate::test_helper::general::init();
        let dao = Arc::new(PassthroughCardDao::new());
        ...
        let found = dao.clone().find_card_for_user_in_status(user.id, PassthroughCardStatus::Open).await.expect("should find");
        assert_eq!(found.id, created_card.id);
    }

    #[test]
    async fn test_find_for_user_in_status_does_not_find() {
        crate::test_helper::general::init();
        let user = create_user().await;
        let expected_date = NaiveDate::from_ymd_opt(2026, 9, 1).expect("date should create");
        let token = Uuid::new_v4();
        ...
        assert_eq!(
            DataError::NotFound("test".into()),
            dao.clone().find_card_for_user_in_status(user.id, PassthroughCardStatus::Closed).await.expect_err("shouldn't find")
        );

        assert_eq!(
            DataError::NotFound("test".into()),
            dao.clone().find_card_for_user_in_status(user.id, PassthroughCardStatus::Paused).await.expect_err("shouldn't find")
        );

        assert_eq!(
            DataError::NotFound("test".into()),
            dao.clone().find_card_for_user_in_status(user.id, PassthroughCardStatus::PendingActivation).await.expect_err("shouldn't find")
        );

        assert_eq!(
            DataError::NotFound("test".into()),
            dao.clone().find_card_for_user_in_status(user.id, PassthroughCardStatus::PendingFulfillment).await.expect_err("shouldn't find")
        );
    }
}